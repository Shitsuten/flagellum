...
- **`exec`** — 在主机上跑 shell 命令。强大但危险,是给爱折腾的人准备的。如果你不需要让 AI 直接操作服务器,**不要启用 exec**,只用 recall 即可。

以下安全段**主要针对 exec**。recall 只需要注意输出脱敏(记忆内容可能含敏感信息,下面的 `sanitizeOutput` 建议也应用于 recall 的返回结果)。

## ⚠️ 安全(针对 exec)

**这套工具(exec)是备用紧急方案,不是日常方案。** 适用场景:官方渠道被封号(包括 app 和网页版)、梯子挂了登录不上、需要临时让 AI 帮忙修服务器。不建议长期使用,原因有二:一是安全——即便加了以下所有防护,exec 的命令和输出仍然经过第三方;二是成本——编程任务带 tools 的 token 消耗大,走官方 key 或 OR 会很烧钱。**系统级任务(修服务器、改配置)能用官方客户端(Claude Code 等)就用官方客户端**,这个方案只在官端不可用时作为备选。
