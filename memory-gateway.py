...
"""
import json, sqlite3, os, sys, re, uuid
...
def get_bm25_scorer():
    ...
    now = datetime.now()
    if _bm25_last_build and (now - _bm25_last_build).total_seconds() < 300:
        return _bm25
    count = _col.count()
    if count == 0:
        return _bm25
    ...
    while offset < count:
        ...
        offset += batch_size
    _bm25.build(all_ids, all_docs)
    _bm25_last_build = now
...
MEM_DIR   = os.environ.get('MEMORY_DIR', './memory')
DB_PATH   = MEM_DIR + '/meta.db'
VEC_PATH  = MEM_DIR + '/vectors'
VAULT_PATH = os.environ.get('VAULT_JSON', './data/vault.json')
...
def search_memories(query, n=5, boost_heat=True, log_access=None):
    ...
    for mid in all_ids:
        ...
        # Three-factor ranking (2026-06-10 simplification):
        #   semantic (vector+BM25) x recency decay x access boost, gated by validity window.
        # heat/tier/energy/polarity/edges are frozen out of ranking.
        vec_score = max(0, 1 - dist)
        bm25_norm = bm25_scores.get(mid, 0) / max_bm25 if max_bm25 > 0 else 0
        semantic = vec_score * 0.70 + bm25_norm * 0.30
    ...
    def do_POST(self):
        ...
        # ── /raw-search — L0 exact-text recall over the raw archive (FTS5) ──
            ...
            if not q:
                return self._json({'error': 'query required'}, 400)
            try:
                rdb = sqlite3.connect(os.path.join(MEM_DIR, 'raw-archive.db'))
