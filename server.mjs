import { handleGatewaySend, heartbeat, getGatewayStats, getContextBlocks, getMcpStatus } from "./gateway.mjs";
...
const readBody = req => new Promise(r => { let d=''; req.on('data',c=>d+=c); req.on('end',()=>r(d)); });
  ...
  // ============================================================
  const MEM_DIR = process.env.MEMORY_DIR || './memory';
...
http.createServer(async (req, res) => {
  ...
  // POST /inject - inject a proactive assistant message + push notification
    ...
    // 推送通知
    try {
      const pushBody = JSON.stringify({ title: 'Hubby', body: content.slice(0, 100), url: '/raffaello/chat/' });
  ...
  // POST /gateway/heartbeat — manual cache heartbeat
  else if (req.method === "POST" && path === "/gateway/heartbeat") {
